...
var urlparse = require("url").parse,
    ...
    //utmlExpress = require("underscore-express"),
    utml = require("utml"),
    fs = require("fs"),
    path = require("path"),
    ...
    rawBody = require("./rawbody").rawBody, //there is a npm version
...
var makeApp = function(configBase, callback) {
    ...
    db.connect({}, function(err) {
        ...
        if (!config.noweb) {
            app.session = session({
                secret: config.secret || "insecure",
                store: dbstore,
                resave: false,
                saveUninitialized: false} //cookie law europe?
                );
        }
        ...
        //app.configure(function() {
            ...
            app.use(bodyParser.raw()); //or bodyParser.raw() (buffer)
            //app.use(express.cookieParser());
            app.use(express.query()); //replace with qs
            //app.use(methodOverride); //use methodOverride('X-HTTP-METHOD'); or others.. custom
            ...
            var defang = function(obj) {
                ...
                _.each(dup, function(value, name) {
                    if (name == "displayName" && _.isString(value)) {
                        console.log(value);
                        //dup[name] = validator.sanitize(value).escape();
